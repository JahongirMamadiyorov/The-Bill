...
export default function OwnerInventory() {
  ...
  const getStockStatus = (quantity, minStock) => {
    if (quantity <= 0) return { badge: 'Out of Stock', color: 'bg-red-100 text-red-700' };
    if (quantity <= minStock) return { badge: 'Low Stock', color: 'bg-yellow-100 text-yellow-700' };
    return { badge: 'In Stock', color: 'bg-green-100 text-green-700' };
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition">
            ...
            <p className="text-xs text-gray-500 mt-2">Items below minimum</p>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-8">
          <div className="p-6 border-b border-gray-200">
            <h2 className="text-lg font-bold text-gray-900">Complete Inventory</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="border-b border-gray-200 bg-gray-50">
                <tr>
                  <th className="text-left py-3 px-6 text-gray-600 font-medium">Item Name</th>
                  <th className="text-left py-3 px-6 text-gray-600 font-medium">Quantity</th>
                  <th className="text-left py-3 px-6 text-gray-600 font-medium">Unit</th>
                  <th className="text-left py-3 px-6 text-gray-600 font-medium">Min Stock</th>
                  <th className="text-left py-3 px-6 text-gray-600 font-medium">Status</th>
                  <th className="text-left py-3 px-6 text-gray-600 font-medium">Supplier</th>
                </tr>
              </thead>
              <tbody>
                {Array.isArray(inventory) && inventory.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan="6" className="text-center py-8 text-gray-500">No inventory items</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="divide-y divide-gray-200">
              {Array.isArray(suppliers) && suppliers.length > 0 ? (
                ...
                <div className="text-center py-8 text-gray-500">No suppliers</div>
              )}
            </div>
          </div>
        </div>
