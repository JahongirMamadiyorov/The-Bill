...
// ── Table status config ────────────────────────────────────────────────────────
const TABLE_STATUS = {
  free:     { color: '#16A34A', bg: '#F0FDF4', border: '#BBF7D0', label: 'Free'     },
  occupied: { color: '#D97706', bg: '#FFFBEB', border: '#FDE68A', label: 'Occupied' },
  reserved: { color: '#7C3AED', bg: '#F5F3FF', border: '#DDD6FE', label: 'Reserved' },
  cleaning: { color: '#0891B2', bg: '#ECFEFF', border: '#A5F3FC', label: 'Cleaning' },
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pr-1">
        ...
        <div className="bg-gray-50 rounded-xl p-4 space-y-1.5 text-sm">
          <div className="flex justify-between text-gray-600">
            <span>Subtotal</span><span className="font-medium text-gray-900">{money(subtotal)}</span>
          </div>
          {taxAmt > 0 && (
            <div className="flex justify-between text-gray-600">
          ...
          {discAmt > 0 && (
            <div className="flex justify-between text-green-600">
              <span>Discount ({discPct}%)</span><span className="font-medium">-{money(discAmt)}</span>
            </div>
          )}
        ...
        {method === 'Cash' && (
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Amount Given</p>
            <input
              type="number"
            ...
            {(parseFloat(cashGiven) || 0) >= total && (
              <div className="mt-2 flex justify-between items-center bg-green-50 border border-green-200 rounded-xl px-4 py-2.5">
                <span className="text-green-700 text-sm font-medium">Change</span>
        ...
        {method === 'Card' && (
          ...
          >
            ...
            <span className="text-sm font-medium text-gray-700">Payment confirmed on terminal</span>
          </button>
        )}
        ...
        {method === 'QR Code' && (
          <div className="space-y-3">
            <div className="flex flex-col items-center justify-center p-6 bg-gray-50 rounded-xl border border-dashed border-gray-300">
              ...
              <p className="text-sm text-gray-500">Customer scans to pay</p>
            </div>
            <button
            ...
            >
              ...
              <span className="text-sm font-medium text-gray-700">QR payment confirmed</span>
            </button>
          </div>
        )}
        ...
        {method === 'Loan' && (
          <div className="space-y-3">
            <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-xl">
              ...
              <p className="text-xs text-amber-700 font-medium">Loan records the debt. Money is collected later.</p>
            </div>
            <input
              type="text"
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
          <div>
            ...
            <p className="text-sm" style={{ color: cfg.color }}>{cfg.label}{table.capacity ? ` · ${table.capacity} seats` : ''}</p>
          </div>
            ...
            hasOrder ? (
              <div className="space-y-4">
                ...
                {billRequested && (
                  <div className="flex items-center gap-2 p-3 rounded-xl border" style={{ backgroundColor: '#FDF4FF', borderColor: '#E9D5FF' }}>
                    ...
                    <p className="text-sm font-semibold" style={{ color: '#7C3AED' }}>Bill requested by waitress</p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { label: 'Order', value: fmtOrderNum(order) },
                    { label: 'Time', value: elapsed(order.createdAt) || '—' },
                    { label: 'Guests', value: order.guestCount ? `${order.guestCount} people` : '—' },
                    { label: 'Server', value: order.waitressName || 'Cashier' },
                  ].map(({ label, value }) => (
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {items.length === 0 ? (
                      <p className="text-sm text-gray-400 py-2">No items loaded</p>
                    ) : items.map((item, i) => (
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-xl border-2 p-4 flex items-center gap-3"
                  ...
                  <div>
                    <p className="text-base font-extrabold" style={{ color: '#7C3AED' }}>Table Reserved</p>
                    <p className="text-xs" style={{ color: '#7C3AED', opacity: 0.75 }}>This table has an upcoming reservation</p>
                  </div>
                </div>
                ...
                {(() => {
                  ...
                  const rows = [
                    { icon: <User className="w-4 h-4" />,          label: 'Guest Name',  value: guest || 'Not specified', faded: !guest },
                    { icon: <Users className="w-4 h-4" />,          label: 'Party Size',  value: seats ? `${seats} seats` : '—', faded: !seats },
                    { icon: <CalendarClock className="w-4 h-4" />,  label: 'Reserved For', value: timeDisplay, faded: !timeRaw },
                    ...(phone ? [{ icon: <Phone className="w-4 h-4" />, label: 'Phone', value: phone, faded: false }] : []),
                  ];
                  return (
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="font-semibold text-gray-700 mb-1">No active order</p>
                <p className="text-sm text-gray-500 mb-6">Start a new order for this table</p>
                <button
                  onClick={() => navigate('/cashier/new-order', { state: { tableId: table.id } })}
      ...
      <div className="flex-shrink-0 bg-white border-b border-gray-200 px-6 py-4"
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-extrabold text-gray-900">Floor Plan</h1>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-4 gap-4">
            {[
              { label: 'Total Tables', val: totalTables,   icon: <TableProperties className="w-5 h-5" />, color: '#0891B2', bg: '#F0F9FF', border: '#BAE6FD' },
              { label: 'Occupied',     val: occupied,       icon: <Activity className="w-5 h-5" />,       color: '#D97706', bg: '#FFFBEB', border: '#FDE68A' },
              { label: 'Free',         val: free,           icon: <Zap className="w-5 h-5" />,            color: '#16A34A', bg: '#F0FDF4', border: '#BBF7D0' },
              { label: 'Reserved',     val: reserved,       icon: <CalendarClock className="w-5 h-5" />,  color: '#7C3AED', bg: '#F5F3FF', border: '#DDD6FE' },
            ].map(s => (
              <div key={s.label}
            ...
            <div className="flex items-center gap-3 px-4 py-3 rounded-xl border-2 border-purple-200 bg-purple-50">
              ...
              <div className="flex-1">
                <p className="text-sm font-bold text-purple-800">
                  {billReqCount} table{billReqCount > 1 ? 's' : ''} waiting for payment
                </p>
                <p className="text-xs text-purple-600">Collect payment from highlighted tables</p>
              </div>
          ...
          {enrichedTables.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-32 text-center">
              ...
              <p className="text-lg font-bold text-gray-400">No tables configured</p>
              <p className="text-sm text-gray-300 mt-1">Add tables from the admin panel</p>
            </div>
          ) : (
                                ...
                                <div className="space-y-1.5">
                                  ...
                                  <div className="px-2 py-1 rounded-lg text-center"
                                    ...
                                    <p className="text-xs font-bold text-white">
                                      {billReq ? 'Bill Requested' : cfg.label}
                                    </p>
                                  </div>
                                </div>
                              ) : (
