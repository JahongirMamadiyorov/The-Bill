...
// ─── API helpers ──────────────────────────────────────────────────────────────
const menuAPI    = { getCategories: () => api.get('/menu/categories'), getItems: () => api.get('/menu/items') };
const tablesAPI  = { getAll: () => api.get('/tables') };
const ordersAPI  = {
...
// MAIN COMPONENT
// ─────────────────────────────────────────────────────────────────────────────
export default function NewCashierPOS({ navigation }) {
  const { user, logout } = useAuth();
  const { width, height } = useWindowDimensions();
  const isLandscape = width > height;

  // ── Portrait tab switch (menu vs order panel) ─────────────────────────────
  ...
  // Landscape: 4 cols on wide tablet, 3 on narrow
  // Portrait:  2 cols on phone, 3 on wide tablet portrait
  ...
  const renderCategoryItem = ({ item: cat }) => {
    ...
    return (
      <TouchableOpacity
        ...
        activeOpacity={0.7}
      >
        <MaterialIcons name={catMIcon(cat.name)} size={18} color={active ? WH : MUT} />
        <Text style={[s.catChipLabel, active && { color: WH }]} numberOfLines={1}>{cat.name}</Text>
      </TouchableOpacity>
    );
  };
  ...
  const renderMenuItem = ({ item }) => {
    ...
    // Dynamic card width based on numCols and screen width
    const panelW = isLandscape ? width - 200 - 320 : width; // subtract sidebar + order panel in landscape
  ...
  const MenuPanel = () => (
    <View style={s.menuPanel}>
            ...
            if (cat.id === null) {
              ...
              return (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <MaterialIcons name="grid-view" size={18} color={active ? WH : MUT} />
                  <Text style={[s.catChipLabel, active && { color: WH }]}>All</Text>
                </TouchableOpacity>
              );
            }
            return renderCategoryItem({ item: cat });
          }}
          horizontal
      ...
      {filteredItems.length === 0 ? (
        ...
        <FlatList
          key={numCols} // forces re-render when columns change
          data={filteredItems}
          keyExtractor={i => String(i.id)}
    ...
    <View style={{ flex: 1, backgroundColor: BG }}>
      ...
      <View style={[s.topBar, { paddingTop: topInset + 4 }]}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          ...
          <View>
            <Text style={{ fontSize: 14, fontWeight: '700', color: TXT }}>The Bill</Text>
            <Text style={{ fontSize: 10, color: MUT }}>POS Terminal</Text>
          </View>
        </View>
        ...
        {/* User info */}
        <Text style={{ fontSize: 12, color: MUT, marginRight: 12 }}>{user?.name}</Text>
        ...
        <TouchableOpacity onPress={() => {
          ...
          <MaterialIcons name="logout" size={18} color={RD} />
        </TouchableOpacity>
      </View>
...
const s = StyleSheet.create({
  ...
  logoutBtn: {
    padding: 6, borderRadius: 8, borderWidth: 1, borderColor: '#FEE2E2', backgroundColor: '#FEF2F2',
  },
  ...
  searchInput: {
    flex: 1, fontSize: 13, color: TXT, padding: 0,
  },
  refreshBtn: {
  ...
  catChipLabel: {
    fontSize: 10, fontWeight: '600', color: TXT, maxWidth: 64,
  },
  sectionHeading: {
  ...
  tableDropdown: {
    ...
    backgroundColor: WH, overflow: 'hidden',
    ...shadow.md,
  },
  tableOption: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
