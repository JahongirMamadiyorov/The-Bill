...
//    since screens live one level deeper than the website's pages/admin/*).
//    `money` now comes from lib/adminFormat.js directly (re-exported by
//    hooks/useApi.js on the website) — same value/behavior either way, this
//    screen never calls useApi() itself (confirmed by grep), so there's no
//    hook dependency lost. This screen's own local `fmtDate`/`fmtDateTime`/
//    `fmtDateStr`/`todayStr` helpers (different format — dd.mm.yyyy display,
//    not adminFormat.js's yyyy-mm-dd) are kept exactly as the source has
//    them, not swapped for the shared ones — they were never the same
//    function on the website either.
// 2. No `navigate` prop taken or used — this screen never calls react-router
...
// to exclude.
//
// window.prompt/confirm/alert: grepped the source for all three — zero
// matches. This screen never uses any of them, unlike Inventory's two real
// window.prompt() bugs — nothing needed the local-modal-replacement pattern
// here.
//
// "Remind overdue" — NOT in this screen, contrary to an earlier assumption.
// `client.js`'s `loansAPI.notifyOverdue()` (POST /loans/notify-overdue)
// exists, but grepping the actual source file for "notify"/"remind"/"Remind"
// turns up zero call sites — AdminLoans.jsx never calls it and has no remind-
// overdue button anywhere. Ported nothing for it; flagging this since the
// build brief expected a remind-overdue flow to exist here.
//
// `loansAPI.getStats()` — same story, re-verified: exists in client.js but
// this screen computes its own KPI totals (total/active/overdue/outstanding)
// client-side from the raw `loansAPI.getAll()` array instead of calling it.
// Confirmed by grep — no `getStats` call site in the source file. The
// earlier research note held up exactly as expected.
// ─────────────────────────────────────────────────────────────────────────────
