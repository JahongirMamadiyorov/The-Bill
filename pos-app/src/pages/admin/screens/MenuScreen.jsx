...
// ─────────────────────────────────────────────────────────────────────────────
// Ported verbatim from website/src/pages/admin/AdminMenu.jsx (same design,
// same data, same computations — "no design/functional changes" rule for
// this whole Admin build), with the following adaptations:
//
// 1. Imports adjusted for pos-app's file layout (screens/ is one level deeper
//    than website's pages/admin/) — same useTranslation/useApi+money/
//    menuAPI+warehouseAPI/invalidateAll already ported and verified for
//    Dashboard/Tables, reused here unchanged.
//
// 2. `ConfirmDialog` didn't exist in pos-app yet (Dashboard/Tables never
//    needed it) — ported verbatim to `src/components/ConfirmDialog.jsx`,
//    only its own `useTranslation` import path adjusted. Used here for the
//    station-delete confirm and the upload-issue/upload-failed notices.
//
// 3. No `useNavigate()`/`navigate` prop adaptation needed — unlike Dashboard
//    and Tables, AdminMenu.jsx never calls react-router navigation anywhere
//    (confirmed by grep), so there's nothing to swap for AdminShell's `goTo`
//    adapter. This screen is self-contained.
//
// 4. Renamed the exported component to `AdminMenuScreen` (source file default-
//    exports `AdminMenu`) purely for clarity against the unrelated cashier-
//    side `pos-app/src/pages/pos/MenuScreen.jsx` — the two never collide on
//    disk (different folders) or at runtime (different SCREENS maps), this
//    is just to avoid any human confusion reading stack traces/dev tools.
//
// Printing: confirmed by grepping the source file for "print"/"Print" —
// zero matches. Nothing to exclude on this screen.
//
// Image upload — real, pre-existing gap, handled here (not built around):
// AdminMenu.jsx's item-photo picker called `menuAPI.uploadImage(file)`,
// which needs multipart/FormData support. pos-app's IPC-based API client
// only speaks JSON today, so `menuAPI.uploadImage` is a deliberate stub that
// throws `'menuAPI.uploadImage: image upload is not yet supported in the
// Admin panel.'` (see client.js's own header comment — this is a known,
// tracked gap, not an oversight, and building real multipart IPC is out of
// scope for this port). Rather than wire the picker to a handler that would
// always throw on the very first real use, the upload control itself is
// rendered in a disabled, clearly-labeled state (amber warning icon +
// "Photo upload not available yet" + a tooltip pointing admins at the
// website Admin panel as the working alternative for now) — `handleImage
// FileChange`/`imageUploading` state from the source file were dropped
// since they'd be permanently unreachable dead code behind a disabled
...
// ── Resolve image URLs ──────────────────────────────────────────────────────
// Two fixes, neither a design change — both make existing/legacy data render
// correctly rather than changing how anything looks:
// 1. Android emulator uses 10.0.2.2 instead of localhost (ported as-is from
//    the website — harmless here, kept for parity in case any stored URL
//    ever has it).
// 2. Backend bug found 2026-07-27 while testing this screen: menu.js's photo
//    upload built `fullUrl` from `req.protocol`, which read 'http' instead of
//    'https' for every request behind Render's proxy (fixed at the source in
//    restaurant-app/backend/src/server.js via `app.set('trust proxy', 1)` —
//    see that file's comment) — but rows already saved with an http:// URL
//    before that fix stay wrong in the database. pos-app's CSP only allows
//    `https://the-bill-backend-pego.onrender.com` for images (deliberately,
//    not http), so those existing rows get silently blocked by the browser
//    instead of just failing to load — confirmed via a live console error
//    ("Refused to load the image ... violates ... img-src"). Upgrading the
//    scheme here (same host, same path, just http->https) fixes every
//    already-stored bad URL immediately, no backend redeploy or data
//    migration needed, and is a no-op for any URL that's already https.
const resolveImgUrl = (url) => {
  if (!url) return '';
  return url
    .replace('http://10.0.2.2:', 'http://localhost:')
    .replace('https://10.0.2.2:', 'https://localhost:')
    .replace('http://the-bill-backend-pego.onrender.com', 'https://the-bill-backend-pego.onrender.com');
};
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5">
              <div className="grid grid-cols-[2fr_3fr] gap-5">
                ...
                <div className="space-y-4">
                  ...
                  {/* Image — preview only. Upload is a known, tracked gap —
                      menuAPI.uploadImage is a stub (see this file's header
                      comment and client.js) because pos-app's IPC layer only
                      speaks JSON, not multipart/FormData, yet. Rather than
                      wire a control that would always throw, it's shown
                      disabled with an explanatory tooltip. The existing-photo
                      preview and its "×" remove button still work fully —
                      removing just clears the local imageUrl field, no
                      upload API involved. */}
                  <div>
                    ...
                    <div
                      className="flex items-center gap-2 px-3 py-2.5 rounded-xl border-2 border-dashed border-gray-200 bg-gray-100 cursor-not-allowed"
                      title={t('admin.menu.uploadNotSupportedHint')}
                    >
                      <AlertTriangle size={15} className="text-amber-500 flex-shrink-0" />
                      <span className="text-sm text-gray-500 font-medium">{t('admin.menu.uploadNotSupportedLabel')}</span>
                    </div>
                  </div>
