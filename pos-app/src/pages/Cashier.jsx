...
import {
  ...
  CheckCircle2, Percent, Hash, Users, Grid3X3, ChevronDown, ChevronUp,
  TableProperties, ArrowLeft,
} from 'lucide-react';
import { camelizeRows } from '../lib/case.js';

// Ported from website/src/pages/new-cashier/NewCashierPOS.jsx (Phase 1 — see
// STATUS.md), with two pieces of behavior pulled in from the OLDER cashier
// screen (website/src/pages/cashier/CashierMenu.jsx) at the project owner's
// request on 2026-07-07:
//   - Weighed items (unit = kg/l/g/ml) get a type-an-amount modal instead of
//     a +/- stepper — see isWeighedItem/openAmountPicker/confirmAmountPicker.
//   - Table selection opens a full card-grid picker (color-coded by status:
//     free/occupied/reserved/cleaning) in place of the menu grid, instead of
//     a plain dropdown — see showTablePicker.
//
// Data layer, same as before:
//   - Reads (menu, categories, tables, active orders) come from the local
//     PowerSync database via window.electronAPI.psGetAll — works offline.
//     Local rows are snake_case with 0/1 booleans; camelizeRows() (src/lib/
//     case.js) translates them.
//   - Writes (Fire / Charge) go straight to the backend via
//     window.electronAPI.ordersCreate/ordersPay — online-required for Phase 1
//     (see the submitOrderWrite() comment in main.js for why, and how offline
//     queuing would slot in later).
//   - Auth is pos-app's session (getSession/logout), not the website's
//     AuthContext; navigation is handled by App.jsx's Guard, not useNavigate.

// ─── Design tokens (identical to website's Cashier) ───────────────────────────
...
// Items priced by weight/volume (kg, l, g, ml) get a type-an-amount modal
// instead of a +/- stepper, matching the older CashierMenu.jsx behavior.
...
// Table status → color/label, matching website/src/pages/cashier/CashierTables.jsx
const tableStatusColor = (status) => {
  if (!status || status === 'free') return { border: '#BBF7D0', bg: '#F0FDF4', badge: '#16A34A', label: 'Free' };
  if (status === 'occupied')        return { border: '#FDE68A', bg: '#FFFBEB', badge: '#D97706', label: 'Occupied' };
  if (status === 'reserved')        return { border: '#DDD6FE', bg: '#F5F3FF', badge: '#7C3AED', label: 'Reserved' };
  if (status === 'cleaning')        return { border: '#A5F3FC', bg: '#ECFEFF', badge: '#0891B2', label: 'Cleaning' };
  return { border: BD, bg: WH, badge: MUT, label: status || '' };
};
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function Cashier({ session, onLogout }) {
  ...
  // ── Weighed-item amount picker (kg / l / g / ml) ───────────────────────────────
  const [amountPicker, setAmountPicker] = useState(null); // { item, draft, priceDraft }
  ...
  const [payInfoOpen, setPayInfoOpen] = useState(true); // Card/QR placeholder box expanded?
  ...
  // ── Load menu data (local PowerSync — works offline) ──────────────────────────
  useEffect(() => { loadMenu(); }, []);
  ...
  const loadOrders = async () => {
    try {
      const rows = await window.electronAPI.psGetAll(
        `SELECT * FROM orders WHERE status IN ('pending','sent_to_kitchen','preparing','ready') ORDER BY created_at DESC`
      );
      ...
      setActiveOrders(orders.map(o => ({
        ...o,
        tableName: o.tableId
          ? (tableMap[o.tableId]?.name || (tableMap[o.tableId]?.tableNumber ? `Table ${tableMap[o.tableId].tableNumber}` : null))
          : null,
      })));
    } catch {}
  };

  // Poll while the Orders tab is open — an order can be created from this same
  // terminal (write goes to the backend, then flows back through PowerSync
  // replication) or from another terminal entirely, so a one-time load isn't
  // enough. Matches the polling pattern already used in the website's
  // CashierTables.jsx (5s) and this app's own RolePlaceholder.jsx (2s).
  useEffect(() => {
  ...
  const decItem = (id) => {
    const entry = cart[id];
    if (!entry) return;
    // Weighed items: reopen the amount picker so the user can lower (or clear) it
  ...
  const clearCart = () => {
    ...
    setSelTable(null); setOrderType('dine_in'); setShowTablePicker(false);
    setCashIn(''); setCustName(''); setCustAddr(''); setError(''); setAmountPicker(null);
    setLoanName(''); setLoanPhone(''); setLoanDueDate('');
  };
  ...
  // ── Shared payload builder (snake_case — matches the Express API exactly) ─────
  ...
  // Dine-in orders must have a table — walk-in-with-no-table only makes sense
  // for To Go / Delivery. Checked before both Fire and Charge.
  const validateOrder = () => {
    if (orderType === 'dine_in' && !selTable) {
      setError('Please select a table for this dine-in order');
      return false;
    }
    if (payMethod === 'Loan' && (!loanName.trim() || !loanDueDate)) {
      setError('Loan payments need the borrower\'s name and a due date');
      return false;
    }
    return true;
  };
  ...
  // Note: a charged order is immediately marked "paid" on the backend, so it will
  // NOT appear under the Orders tab — that tab only lists still-in-progress
  // orders (pending/sent_to_kitchen/preparing/ready), same as the website. Paid
  // orders belong in History/Bills, which are not built yet in this phase.
  const handleCharge = async () => {
    ...
    try {
      ...
      if (!createRes.ok) { setError(createRes.error || 'Failed to create order'); return; }
      const orderId = createRes.data?.id;
      const payPayload = payMethod === 'Loan'
        ? {
            payment_method:      'Loan',
      ...
      const payRes = await window.electronAPI.ordersPay(orderId, payPayload);
      if (!payRes.ok) { setError(payRes.error || 'Payment failed — order was created but not marked paid, check Orders tab'); return; }
  ...
  // ── Logout ────────────────────────────────────────────────────────────────────
  const handleLogout = async () => { await window.electronAPI.logout(); onLogout(); };
        ...
        <>
              ...
              <>
                <div style={{
                  ...
                  <button onClick={() => setShowTablePicker(false)} style={{
                    ...
                    <ArrowLeft size={15} color={TXT} /> Back to menu
                  </button>
                  <div style={{ fontSize: 15, fontWeight: 700, color: TXT }}>Select a table</div>
                  <div style={{ flex: 1 }} />
                  <div style={{ fontSize: 12, color: MUT }}>{tables.length} tables</div>
                </div>

                <div style={{ flex: 1, overflowY: 'auto', padding: '20px 22px' }}>
                  {/* Walk-in option */}
                  <div style={{ marginBottom: 18 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: MUT, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 10 }}>No table</div>
                    <button onClick={() => { setSelTable(null); setShowTablePicker(false); }} style={{
                      ...
                      <div>
                        <div style={{ fontSize: 14, fontWeight: 700, color: selTable === null ? C : TXT }}>Walk-in / No table</div>
                        <div style={{ fontSize: 11, color: MUT, marginTop: 1 }}>Takeaway counter</div>
                      </div>
                  ...
                  {/* Tables by section */}
                  {tables.length > 0 && (() => {
                    const sectionNames = [...new Set(tables.map(tbl => tbl.section || 'Main Floor'))];
                    return sectionNames.map(sectionName => {
                      const sectionTables = tables.filter(tbl => (tbl.section || 'Main Floor') === sectionName);
                      return (
                                  ...
                                  <div>
                                    {tbl.capacity && (
                                      <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 4 }}>
                                        ...
                                        <span style={{ fontSize: 10, color: MUT }}>{tbl.capacity} seats</span>
                                      </div>
                                    )}
                                    <div style={{
                                      ...
                                      <span style={{ fontSize: 10, fontWeight: 600, color: sc.badge }}>{sc.label}</span>
                                    </div>
                                    {tbl.status === 'occupied' && (
                                      <div style={{ fontSize: 9, fontWeight: 600, color: sc.badge, marginTop: 4, lineHeight: 1.3 }}>
                                        Selecting adds to this table's next order
                                      </div>
                                    )}
                                  </div>
                                </button>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div style={{ height: 108, background: '#F3F4F6', position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
                              ...
                              {qty > 0 && (
                                <div style={{
                                  position: 'absolute', top: 7, right: 7, minWidth: 22, height: 22,
                                  borderRadius: 11, background: C, color: WH,
                                  fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center',
                                  padding: weighed ? '0 6px' : 0,
                                }}>{weighed ? formatQty(item, qty) : qty}</div>
                              )}
                            </div>
                            ...
                            <div style={{ padding: '9px 10px 11px', flex: 1, display: 'flex', flexDirection: 'column', gap: 3 }}>
                              ...
                              {qty === 0 ? (
                                ...
                                <div style={{
                                  ...
                                  <button onClick={() => openAmountPicker(item)} style={{ fontSize: 12, fontWeight: 700, color: C, border: 'none', background: 'none', cursor: 'pointer', padding: '0 2px' }}>
                                    {formatQty(item, qty)}
                                  </button>
                                  <button onClick={() => openAmountPicker(item)} style={{
                                    width: 26, height: 26, borderRadius: 6, border: 'none',
          ...
          <div style={{
            ...
            <div style={{ padding: '11px 16px 12px', borderBottom: `1px solid ${BD}`, flexShrink: 0 }}>
              ...
              {/* Table card button (dine_in) — opens the full table picker.
                  Required for dine_in orders — see handleFire/handleCharge validation. */}
              {orderType === 'dine_in' && (
                <button onClick={() => setShowTablePicker(true)} style={{
                  width: '100%', padding: '10px 12px', borderRadius: 10,
                  border: `2px solid ${selTable ? C : (error && !selTable ? RD : BD)}`,
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: selTable?.status === 'occupied' ? RD : MUT, marginTop: 1 }}>
                      {selTable
                        ? tableStatusColor(selTable.status).label
                        : 'Tap to choose a table'}
                    </div>
                  </div>
                  <ChevronDown size={14} color={selTable ? C : MUT} style={{ transform: 'rotate(-90deg)' }} />
                </button>
              )}
                ...
                cartEntries.map(({ item, qty }) => (
                  <div key={item.id} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                      ...
                      <span style={{ fontSize: 13, fontWeight: 700, color: TXT, minWidth: 18, textAlign: 'center' }}>
            ...
            {cartEntries.length > 0 && (
              <div style={{ flexShrink: 0, borderTop: `1px solid ${BD}`, padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 9 }}>
                ...
                {/* Loan — who owes the money and when it's due (required by the backend) */}
                {payMethod === 'Loan' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                    <input
                    ...
                    />
                    <input
                      value={loanPhone} onChange={e => setLoanPhone(e.target.value)}
                      placeholder="Phone (optional)..."
                      style={{ padding: '8px 10px', border: `1px solid ${BD}`, borderRadius: 8, fontSize: 13, outline: 'none', color: TXT }}
                    />
                    <div>
                      <div style={{ fontSize: 11, color: MUT, marginBottom: 4 }}>Due date</div>
                      <input
                        type="date" value={loanDueDate} onChange={e => setLoanDueDate(e.target.value)}
                        min={new Date().toISOString().slice(0, 10)}
                        style={{ width: '100%', padding: '8px 10px', border: `1px solid ${BD}`, borderRadius: 8, fontSize: 13, outline: 'none', color: TXT, boxSizing: 'border-box' }}
                      />
                    </div>
                  </div>
                )}

                {/* Card / QR — reserved for a future payment terminal integration.
                    Today this just tags the order with the method; no external
                    payment API is wired up yet. Collapsible so it doesn't take
                    permanent space once you've seen it. */}
                {(payMethod === 'Card' || payMethod === 'QR Code') && (
                  <div style={{ borderRadius: 8, border: `1.5px dashed ${BD}`, background: '#F9FAFB', overflow: 'hidden' }}>
                    <button
                      onClick={() => setPayInfoOpen(p => !p)}
                      style={{
                        width: '100%', padding: '8px 12px', border: 'none', background: 'none',
                        cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                        fontSize: 12, fontWeight: 600, color: MUT,
                      }}
                    >
                      <span>{payMethod} integration status</span>
                      {payInfoOpen ? <ChevronUp size={14} color={MUT} /> : <ChevronDown size={14} color={MUT} />}
                    </button>
                    {payInfoOpen && (
                      <div style={{ padding: '0 12px 10px', fontSize: 12, color: MUT, textAlign: 'center' }}>
                        {payMethod} terminal integration — not connected yet. Order will be marked
                        paid by {payMethod.toLowerCase()} without an external payment check.
                      </div>
                    )}
                  </div>
                )}
        ...
        <div style={{ flex: 1, padding: 24, overflowY: 'auto' }}>
          ...
          {activeOrders.length === 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 320, color: MUT }}>
              ...
              <div style={{ fontSize: 12, marginTop: 6, maxWidth: 320, textAlign: 'center' }}>
                Only Fired (sent to kitchen, unpaid) orders show here. Charged orders are marked
                paid immediately and won't appear in this list.
              </div>
            </div>
          ) : (
            ...
            <div style={{ padding: '18px 20px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                <div style={{ fontSize: 11, fontWeight: 600, color: MUT, marginBottom: 6 }}>
                  Quantity ({unitSuffix(amountPicker.item)})
                </div>
